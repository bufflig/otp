			  ...
			  int build_result)
{
	...
	if (build_result) {
#ifdef HAVE_DTRACE
	    old_value = (SEQ_TRACE_TOKEN(p) == am_have_dt_tag) ? NIL : SEQ_TRACE_TOKEN(p);
#else
	    old_value = SEQ_TRACE_TOKEN(p);
#endif
	}
#ifdef HAVE_DTRACE
        SEQ_TRACE_TOKEN(p) = (DT_UTAG(p) != NIL) ? am_have_dt_tag : NIL;
#else
...
#ifdef HAVE_DTRACE
	|| SEQ_TRACE_TOKEN(BIF_P) == am_have_dt_tag
#endif
	) {
	BIF_RET(am_false);
    }
